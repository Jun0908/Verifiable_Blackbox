...
export function AnalysisPanel({scene}:{scene:Scene}) {
  ...
  return <section className="panel stg-analysis" aria-busy={busy}>
    {publicPreview ? <p className="stg-muted">{t("Saved results are shown below. Reanalysis is available in the local demo.","保存済みの解析結果を表示しています。再解析はローカルデモで実行できます。")}</p> : <div className="stg-toolbar"><button disabled={busy||service!=="ready"} onClick={()=>void analyze()}>{busy?t("Analyzing…","解析中…"):t("Reanalyze recovered video","復元映像を再解析")}</button>
