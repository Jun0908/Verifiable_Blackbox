...
export function SiteHeader({onOverview}: {onOverview?: () => void} = {}) {
  ...
  return <header className="topbar">
    ...
    <div className="header-tools"><nav aria-label={t("Main navigation", "メインナビゲーション")}><Link href="/" onClick={onOverview ? e => {e.preventDefault(); onOverview();} : undefined} aria-current={path === "/" ? "page" : undefined}>{t("Overview", "概要")}</Link><Link href="/rover" aria-current={path === "/rover" ? "page" : undefined}>{t("Operate", "操作")}</Link><Link href="/ledger" aria-current={path === "/ledger" ? "page" : undefined}>{t("Ledger", "台帳")}</Link><Link href="/stegavar" aria-current={path === "/stegavar" ? "page" : undefined}>StegaVAR</Link></nav>
