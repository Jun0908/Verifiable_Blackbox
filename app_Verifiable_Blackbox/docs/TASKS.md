...
- [ ] 前進デモと静止デモを用意する。
- [ ] VIDEOでは録画開始と開始前の静止区間を確認してから操作する。
- [ ] 前進デモでは設定時間の操作後に停止し、静止デモでは同じ時間だけ待機する。VIDEOでは両方を録画する。
- [ ] SKIP_VIDEOでは録画を任意とし、カメラや解析サービスの準備を待たずに操作できるようにする。
- [ ] Bridge側でsessionId、指令連番、方向・出力設定、送信時刻・結果を記録する。
- [ ] 停止要求と出力ゼロ等の停止確認の根拠を保存する。
- [ ] VIDEOでは停止後の映像を取得して録画を終了する。
- [ ] カメラ切断・録画不足を記録し、VIDEOでは支払いを停止する。SKIP_VIDEOでは任意録画の失敗を走行・支払いの阻害条件にしない。
- [ ] 指令送信失敗・停止未確認は両モードで支払いを停止する。
- [ ] 録画と操作記録をJob・sessionIdへ結び付け、非公開領域へ保存する。
- [ ] 緊急停止と通信断時の停止を維持し、開始要求の再送で再走行しないようにする。
- [ ] Roverと使用するカメラをセッション単位で占有し、別Job・自由操作との競合を防ぐ。緊急停止は常に受け付ける。
