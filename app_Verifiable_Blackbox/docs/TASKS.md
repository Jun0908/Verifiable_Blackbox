...
- [ ] 月次集計デモのタブ、対象月、取引先別集計、内訳表示を追加する。
- [ ] 集計CSV・明細CSVをダウンロードできるようにする。ファイル名と各行にサンプルの出所を含める。
- [ ] Asia/Tokyoの月境界、0件、整数計算、CSVのエスケープと数式対策を検証する。
- [ ] 実支払いと月次サンプルを別集計にし、支払済みJobを未払いとして再計上しない。
