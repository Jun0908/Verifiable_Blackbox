...
extern WifiProfiles wifiProfiles;

static esp_err_t network_handler(httpd_req_t *req) {
  char token[sizeof(ROVER_API_TOKEN) + 1] = {};
  if (strlen(ROVER_API_TOKEN) == 0 ||
      httpd_req_get_hdr_value_str(req, "X-Rover-Token", token, sizeof(token)) != ESP_OK ||
      strcmp(token, ROVER_API_TOKEN) != 0) {
    httpd_resp_set_status(req, "401 Unauthorized");
    return httpd_resp_send(req, "unauthorized", HTTPD_RESP_USE_STRLEN);
  }
  httpd_resp_set_type(req, "application/json");
  if (req->method == HTTP_POST) {
    char body[64] = {}, profile[16] = {};
    ...
    if (httpd_query_key_value(body, "profile", profile, sizeof(profile)) != ESP_OK ||
        (strcmp(profile, "home") != 0 && strcmp(profile, "hotspot") != 0) ||
        !wifiProfiles.select(strcmp(profile, "hotspot") == 0)) {
      return httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "network profile not configured");
    }
    httpd_resp_set_status(req, "202 Accepted");
    return httpd_resp_send(req, "{\"ok\":true,\"switching\":true}", HTTPD_RESP_USE_STRLEN);
  }
  const String body = String("{\"ok\":true,\"device\":\"camera\",\"profile\":\"") +
...
static esp_err_t index_handler(httpd_req_t *req) {
  ...
  if (s != NULL) {
    if (s->id.PID == OV3660_PID) {
      ...
      return httpd_resp_send(req, (const char *)index_mega_ccm_html_gz, index_mega_ccm_html_gz_len);
    } else {
...
void startCameraServer() {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    network_get.uri = "/network";
    network_get.method = HTTP_GET;
    network_get.handler = network_handler;
    httpd_register_uri_handler(camera_httpd, &network_get);
    httpd_uri_t network_post = network_get;
    network_post.method = HTTP_POST;
    httpd_register_uri_handler(camera_httpd, &network_post);
