...
1タスクごとに、動く範囲、確認結果、残った制約が説明できる変更にする。大きければタスク内のチェック項目でcommitを分ける。完了したチェックを更新し、公開用の短い記録を`docs/build-log/`へ残す。
...
| 通しデモと公開準備 | T11–T12 | 再現手順、確認記録、短いデモ動画 |
...
公開記録: 2画面の画像と「この段階はUI sample」という説明。
...
公開記録: Core／Hook／Evaluatorの役割図、成功と改ざん拒否の試験結果。
...
公開記録: 最小の「検証可能な記録→支払い」動画。Mockであることを明示。
...
公開記録: 承認文書で保証できることと、物理作業をまだ保証しないこと。復旧の実演。

## T06 — Job進捗・履歴・領収書

依存: T05。主な成果物: `apps/web/lib/`のJob・履歴・機体情報管理、Dashboardの進捗・履歴・領収書コンポーネント。

- [ ] 選択中Jobと保存履歴をWallet・Chain・Coreごとに分離する。
- [ ] 作成Tx、Job、commitment、検証元、Receipt、支払いTxを表示する。
- [ ] 完了済み・サンプル・別WalletのJobを実機仕事として引き継がない。
- [ ] 新しいJobに切り替えても前のJobを履歴に残す。画面操作だけで返金・完了した扱いにしない。
- [ ] 登録済み機体のENS名／鍵ID表示と、当該Jobの署名照合結果を分ける。照合していないJobは「未照合」とする。

完了条件: 再読込・Wallet切替・Job切替で誤った履歴や支払い状態を表示しない。保存したJobを開いた際にChainの最新状態と照合する。

公開記録: 一つのJobからEvidenceと支払いTxまで追える画面。
...
公開記録: Mock／LOCAL_DEV／実Phalaの違いと、確認できた検証範囲。
...
公開記録: 操作画面と停止の仕組み。実機を使った場合は対象と確認内容。

## T09 — カメラとグリッパー

依存: T08。主な成果物: カメラ・グリッパーのUI、camera／control API、Bridge接続試験。

- [ ] BridgeのJPEGを中継し、カメラURL設定・ON/OFFを用意する。
- [ ] 古いフレーム・切断・タブ非表示では映像を消す。取得エラーでも停止操作は使えるようにする。
- [ ] 「はなす」は1回、「つかむ」は長押し更新、ボタン解放は`release`へ接続する。
- [ ] 走行・アームで同じsession／sequenceと停止処理を使う。アームだけで走行済みフラグを立てない。
- [ ] カメラのONや表示だけでARMしない。録画機能は対象外とする。

完了条件: 模擬カメラ／Bridgeで画像更新・切断表示・開閉／解放を確認できる。実機の受信確認と実際の動作確認を区別して記録する。

公開記録: 操作画面の短い動画。カメラ表示は物理作業の自動検証ではないと説明する。
...
公開記録: ENS→公開鍵→Job署名照合。署名が物理作業の完了を証明しないことも併記する。

## T11 — 全体フローの通し確認

依存: T07・T08・T09・T10。

- [ ] Contract・Web・設定・保存先を明示して起動する検証コマンドを整備する。
- [ ] 型チェック・build・Contract試験・承認フロー・Bridge／camera／gripper・独立署名の必要な回帰を実行する。
- [ ] LocalでJob作成→操作終了→承認署名→Phala LOCAL_DEV→支払い→Receiptを確認する。機体がなければ模擬入力を明記する。
- [ ] 改ざん、未承認、別Wallet、署名不一致、期限切れ、Phala停止、二重要求、再起動後の再開、停止失敗を確認する。
- [ ] 実環境ではPhala／Sepoliaと実機を使った同じ導線を確認し、Job・Tx・接続設定・確認時刻を記録する。未確認項目を成功扱いにしない。
- [ ] 「動かない機体」のデモは未承認・未払いとして説明する。静止の自動検出で拒否する機能は未実装と明示する。

完了条件: Localの正常／異常系が通り、実機操作から利用者の承認・実Phala検証・Sepolia支払い・Receipt表示まで確認できる。環境未用意の場合はその項目を未完了で残す。

公開記録: 確認環境・入力の出所・結果・Txをまとめた検証表。公開版から秘密と個人情報を除く。

## T12 — ドキュメントとデモの仕上げ

依存: T11。途中のbuild-logは各タスクで作成する。

- [ ] READMEにセットアップ・起動方法・承認から支払いまでの操作手順を記載する。
- [ ] 設計・タスク・build-logから、実装した機能と未実装の範囲を追えるようにする。
...
- World／StegaVAR／Curvegridの内包、大規模DB化、公開インターネットからのRover操作。
