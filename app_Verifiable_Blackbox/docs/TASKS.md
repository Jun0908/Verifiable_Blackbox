...
- [x] 大きな前ボタンを押すだけで接続・録画・操作を開始する。離すと停止する。
- [x] 約1秒の短い押下も保存する。接続中に離した場合は遅れて前進させない。
...
- [x] 条件確認・署名・記録開始のボタンと速度・時間の入力欄を操作画面から外す。
...
確認: セッション・解析・ログイン照合・支払い条件の22試験、Pythonの18試験、模擬Bridge・実StegaVAR・専用Anvilのブラウザー試験が成功。約1秒の押下、押下なし、接続中の解放、MOVINGとSTILLの参考表示、スキップ、解析サービス停止中の支払い、再送・再読み込みを確認。操作時のpersonal_sign呼び出しは0回。型チェック・production buildも成功。
...
- [ ] Job詳細・領収書から「映像の開示を依頼」で開示サービスへ移動できるようにする。
- [ ] 対象Job・映像・依頼者・5分間の閲覧期限を承認画面に表示する。
- [ ] 承認者コードと対象Jobへの権限を確認し、World認証とは分けて扱う。
- [ ] 明示的な承認操作と、その依頼に対するWorld認証結果を結び付ける。
- [ ] 許可を対象Job・映像ハッシュ・依頼者セッションへ限定する。
- [ ] 所有者自身の録画閲覧と、第三者への開示を区別する。
- [ ] 日英表示、拒否、認証キャンセル、再依頼の導線を用意する。
...
- [ ] 映像を非公開領域から配信し、通常取得・Range要求の両方で権限を確認する。
- [ ] 承認後5分間だけ依頼者セッションへ配信する。
- [ ] 未承認、別セッション、拒否、期限切れ、取消後の配信を拒否する。
- [ ] 動画、復元フレーム、サムネイル、ダウンロード経路からの迂回を確認する。
- [ ] 取消・期限切れ時に画面を閉じ、以後の配信を停止する。
- [ ] 再起動時には開示依頼・認証途中の状態・閲覧許可を失効させる。
- [ ] 公開StegaVARサンプルと保護対象のJob映像を分離する。
...
画面は前ボタンを中心とする。支払い条件は今回のJobの押下記録とし、動画判定は参考表示とする。所有者確認・Jobとの紐付け・二重払い防止はServerで処理する。離した際の停止、通信断時の停止、最大3秒の連続走行上限を維持する。
