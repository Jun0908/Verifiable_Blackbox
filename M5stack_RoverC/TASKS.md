...
- [x] `--web`で起動するHTML・CSS・JavaScript、日英切替、接続・方向・速度・停止UIを作る。
- [x] 長押し更新、pointer解放・取消、タブ非表示・ページ離脱時の停止を接続する。
- [x] グリッパーの開く・段階的に閉じる・releaseを同じsession／sequenceへ接続する。
- [x] 受信確認後に次の角度へ進み、更新切れや解放では閉じ増しを止める。
- [x] Host・Origin・token・本文上限を検査する。
...
- [x] PySide6画面、自動探索・接続・ARM、接続状態、Stop、日英切替を作る。
- [x] pygame-ceによる入力読取を分離し、250msの入力期限と中立確認を実装する。
- [x] 前後・横・斜め・旋回、速度3段階、gripper、Stopの割当と設定画面を作る。
- [x] GUIの短押し・長押し、1°閉じ増し、3°ゆるめる、把持位置保存・呼出を実装する。
- [x] 接続直後のゼロ入力、Telemetry切れ、Controller切断、GUI終了時の停止を試験する。
...
- [x] ブラウザで音声なしWebM録画、REC表示、経過時間、停止操作を提供する。
- [x] カメラOFF・切断・タブ非表示・10分／容量上限で録画を終了する。
- [x] 認証付き保存APIでContent-Type・サイズ・WebM headerを確認し、Server側のファイル名で保存する。
- [x] 不完全なuploadを処理し、保存失敗時にブラウザ保存の導線を出す。
...
- [x] Roverとカメラにprofileの保存・読取・認証付き明示切替APIを実装する。
- [x] 切替前に機体を停止し、ネットワーク処理中も停止処理を継続する。
- [x] WindowsのPC接続先選択と両機器の探索・疎通確認を専用moduleへ分ける。
- [x] USBシリアルによるprofile選択と、他OSでの手動接続手順を用意する。
- [x] profile未設定、接続先不在、DHCP待ち、片方だけ接続できた状態を区別する。
