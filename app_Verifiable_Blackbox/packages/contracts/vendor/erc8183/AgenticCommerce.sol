...
/// @notice Snapshot of the ERC-8183 draft reference implementation used by this MVP.
/// @dev Source provenance is recorded in SNAPSHOT.md next to this file.
    ...
    UUPSUpgradeable
{
    ...
    function _beforeHook(address hook, uint256 jobId, bytes4 selector, bytes memory data)
        internal
    {
        if (hook != address(0)) IACPHook(hook).beforeAction(jobId, selector, data);
    }

    function _afterHook(address hook, uint256 jobId, bytes4 selector, bytes memory data)
        internal
    {
        ...
        nonReentrant
    {
        ...
        uint256 platformFee = (amount * platformFeeBP) / 10_000;
        uint256 evalFee = (amount * evaluatorFeeBP) / 10_000;
        ...
        nonReentrant
    {
        ...
        if (
            (previousStatus == JobStatus.Funded || previousStatus == JobStatus.Submitted)
                && job.budget > 0
        ) {
