...
export async function verifyPrivyOwner(token: string, appId: string, owner: Address,
  key?: JWTVerifyGetKey, fetcher: typeof fetch = fetch) {
  if (!token || token.length > 16384 || !/^[a-z0-9_-]+$/i.test(appId)) throw Error("LOGIN_REQUIRED");
  if (!key) {
    key = keys.get(appId);
  ...
  if (!response.ok) throw Error("LOGIN_REQUIRED");
...
export async function requireJobOwner(request: Request, owner: Address) {
  ...
  await verifyPrivyOwner(token, process.env.NEXT_PUBLIC_PRIVY_APP_ID || "", owner);
}
