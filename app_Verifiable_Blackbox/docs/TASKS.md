...
[ARCHITECTURE.md](ARCHITECTURE.md)に基づき、開発環境、画面、Contract、Evidence検証、決済、実機操作、支払台帳・月次集計、StegaVARを段階的に実装する。
...
- [ ] 動いているケースと停止しているケースを選定する。
- [ ] 各ケースのカバー、埋込み後、差分、復元映像を配置する。
- [ ] manifestと保存済み解析結果を揃える。
- [ ] 参照先、フレーム数、ハッシュの整合性を確認する。
- [ ] 不要な中間生成物を含めない。
...
- [ ] `/stegavar`を追加し、共通メニューから移動できるようにする。
- [ ] 日英切替に対応する。
- [ ] ケースとカバー映像の選択を実装する。
- [ ] カバー・埋込み後・差分の表示切替を実装する。
- [ ] 復元映像の表示、再生・停止・シークを実装する。
- [ ] 保存済み解析結果、解析方式、出典を表示する。
- [ ] Revealが事前復元した映像の表示であることを説明する。
...
- [ ] healthとanalyzeのRoute Handlerを追加する。
- [ ] Pythonの接続先をサーバー側の環境変数で管理する。
- [ ] ケースIDとシーンIDを許可一覧で検証する。
- [ ] Pythonの応答を共通形式へ整形する。
- [ ] 解析中、未起動、失敗、タイムアウトを区別する。
- [ ] 同時解析1件の制限を維持する。
...
- [ ] 保存済み結果と今回の結果を区別する。
- [ ] 実行日時、処理時間、解析方式を表示する。
- [ ] 応答のケースIDと入力ハッシュを照合する。
- [ ] ケース切替後の古い応答による上書きを防ぐ。
- [ ] 失敗時も映像と保存済み結果を維持する。
- [ ] 状態を確認して手動で再試行できるようにする。
...
- [ ] StegaVARを含めて起動できるコマンドを用意する。
- [ ] Python環境、データ、必要なモデルの有無を確認する。
- [ ] ポート競合と起動済みサービスを識別する。
- [ ] Pythonが起動できない場合も映像閲覧を可能にする。
- [ ] 起動処理が開始したプロセスを終了できるようにする。
- [ ] CPU構成、設定項目、データ生成手順をREADMEに記載する。
...
- World認証による映像の開示制御、映像ハッシュとJob・Phalaの結び付け。

これらはT01〜T24の対象外とし、個別に設計する。
