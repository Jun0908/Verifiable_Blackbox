...
**前ボタン記録・Raw動画の3択判定・画面表示・条件付き支払いを実装済み。模擬Bridgeとローカルチェーンで通常・スキップの支払いを確認。実機・Privy署名・実Phala・Sepolia決済の有人通し確認はT32に残る。**
...
- [x] 保存済みの操作記録・所有者承認・動画判定を証拠としてまとめ、Phala検証と支払いAPIへ接続する。
- [x] 下表の条件で支払う。判定後のスキップは同じ記録への追加承認で受け付け、再走行を要求しない。
- [x] 対象Job・署名・記録の整合性・期限・未決済状態をServerで確認する。送金結果不明時はTransactionを照合し、二重払いを防ぐ。
- [x] 支払い結果とReceiptを画面に表示する。動画判定の表示は変更しない。
...
T29確認: 15件のセッション・支払い条件試験とブラウザー試験が成功。模擬BridgeのRaw動画を実StegaVARへ送り、MOVINGで支払い、押下なしのSTILLで保留、INCONCLUSIVEへの追加承認で支払いを確認。開始前スキップも支払い完了。再送・再読み込みによる追加Transactionなし、Provider残高は3件分の予算と一致。チェーンと署名は専用Anvil・公開テスト鍵を使用。

## T32 — 実機で通し確認する

実機は動かさず、実装と模擬試験まで進める指定のため、次の有人確認は未完了。

準備確認（2026-09-26）: WebのPrivyログイン入口、Sepolia設定、Bridgeのidle状態、StegaVARのRaw動画受付、Phala attestation応答（PHALA_DSTACK・attested=true）を確認。型チェックとproduction buildも成功。起動設定の`ROVER_PYTHON_ROOT`を同じリポジトリ内のBridgeへ合わせ、Web・Bridge・StegaVARを起動済み。秘密設定はGit対象外。

有人確認は`http://127.0.0.1:3000`でログインし、Rover Jobを作成・入金して条件確認へ進む。署名が必要な画面は利用者が操作する。条件付き承認の有効時間は最大15分なので、実機操作の直前に署名する。設定アイコンの長押しで動画認識スキップを選べる。今回の準備では実機走行・Privy署名・Sepolia送金は実行していない。
